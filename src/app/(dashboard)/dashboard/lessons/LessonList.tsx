...
export default async function ClassList({ where, studentId, enroled = false }: { where?: LessonTermWhereInput, studentId?: string, enroled?: boolean }) {
    const context = await getSessionContext();
    const { lessonTerms } = await context.graphql.run({ query: GET_LESSONS, variables: { where } })
    return (
        <>
            {!lessonTerms || lessonTerms.length === 0 ? (<div className="px-4 py-5 sm:px-6"> No lessons Found</div>) : (
                                ...
                                <div className="px-4 py-4 sm:px-6">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="ml-2 flex flex-shrink-0">
                                            <p className="inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800">
                                                {lesson.lesson?.lessonCategory?.type ? labelHelper(lessonTypeOptions, lesson.lesson?.lessonCategory?.type) : 'Other'}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="mt-2 sm:flex sm:justify-between">
                                        ...
                                        <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                                            ...
                                            <p>
                                                Starts on <time dateTime={new Date(lesson.term?.startDate).toUTCString()}>{formatDate(lesson.term?.startDate)}</time>
                                            </p>
                                        </div>
                                    </div>
                                </div>
